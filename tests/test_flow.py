...
def _review(passed: bool, issues: list[str] | None = None, feedback: str = "") -> ReviewOutput:
    return ReviewOutput(
        passed=passed,
...
def _tests(passed: bool, failed: int = 0, failures: list[str] | None = None) -> TestOutput:
    return TestOutput(
        passed=passed,
        total_tests=10,
        failed_tests=failed,
...
def _setup(mock_cls, *, raw: str = "OK", pydantic=None, side_effects: list | None = None):
...
def test_happy_path(m_res, m_plan, m_exec, m_rev, m_test, m_dep):
    ...
    flow.kickoff(inputs={"feature_request": "Add hello endpoint", "project_path": "./output"})
...
def test_review_loop_one_retry(m_res, m_plan, m_exec, m_rev, m_test, m_dep):
    ...
    _setup(m_rev, side_effects=[
        _crew_result(pydantic=_review(passed=False, feedback="Missing error handling")),
    ...
    flow = DevWorkflowFlow()
    flow.kickoff(inputs={"feature_request": "Add user auth", "project_path": "./output"})
...
def test_test_loop_one_retry(m_res, m_plan, m_exec, m_rev, m_test, m_dep):
    ...
    flow = DevWorkflowFlow()
    flow.kickoff(inputs={"feature_request": "Add JWT tokens", "project_path": "./output"})
...
def test_review_escalation(m_exec, m_rev):
    ...
    _setup(m_exec, raw="code")
    _setup(m_rev, side_effects=[
        _crew_result(pydantic=_review(passed=False, feedback="Critical bug")) for _ in range(10)
    ])
...
def test_test_escalation(m_exec, m_rev, m_test):
    ...
    _setup(m_exec, raw="code")
    _setup(m_rev, side_effects=[
        _crew_result(pydantic=_review(passed=True)) for _ in range(20)
    ])
    _setup(m_test, side_effects=[
    ...
    assert s.test_retry_count == s.max_test_retries,  "Must retry exactly max_test_retries times"
    assert len(s.errors) > 0,                         "Must record the escalation error"
...
def test_feedback_forwarded_to_executor(m_res, m_plan, m_exec, m_rev, m_test, m_dep):
    ...
    _setup(m_rev, side_effects=[
        _crew_result(pydantic=_review(passed=False, feedback="Add input validation")),
        _crew_result(pydantic=_review(passed=True)),
    ])
    _setup(m_test, pydantic=_tests(passed=True))
    _setup(m_dep,  raw="deployed")
